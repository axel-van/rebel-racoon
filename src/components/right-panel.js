...
import { getThread, subscribe as subscribeThread } from "../assistant.js?v=92";
import { isFlagOn } from "../feature-flags.js?v=19";
import { ideas as MOCK_IDEAS } from "../mocks.js?v=88";
...
} from "../posts-store.js?v=65";
import { renderPostCard } from "./post-card.js?v=104";
import { renderTopPostEcho } from "./top-post-card.js?v=118";
import { renderClipCard } from "./clip-card.js?v=48";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=87";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=344";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=95";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=57";
import { askConnector } from "../connector-ask.js?v=37";
import { renderConnectorLogo } from "../connectors-view.js?v=39";
import { open as openConnectorsModal } from "./connectors-modal.js?v=41";
import { addMention as addComposerMention } from "../composer-mentions.js?v=60";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=88";
import { open as openImageStudioModal } from "./image-studio/index.js?v=103";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=81";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=65").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=609").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=33").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=38"), import("../folders-store.js?v=33")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=609").then(({ askAngleQuestion }) => {
