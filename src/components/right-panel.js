...
import { ideas as MOCK_IDEAS } from "../mocks.js?v=93";
...
} from "../posts-store.js?v=71";
import { renderPostCard } from "./post-card.js?v=113";
import { renderTopPostEcho } from "./top-post-card.js?v=135";
import { renderClipCard } from "./clip-card.js?v=57";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=95";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=420";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=104";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=63";
import { askConnector } from "../connector-ask.js?v=43";
import { renderConnectorLogo } from "../connectors-view.js?v=45";
import { open as openConnectorsModal } from "./connectors-modal.js?v=47";
import { addMention as addComposerMention } from "../composer-mentions.js?v=69";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=97";
import { open as openImageStudioModal } from "./image-studio/index.js?v=113";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=91";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=71").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=685").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=39").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=44"), import("../folders-store.js?v=39")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=685").then(({ askAngleQuestion }) => {
