...
import { getThread, subscribe as subscribeThread } from "../assistant.js?v=77";
import { isFlagOn } from "../feature-flags.js?v=18";
import { ideas as MOCK_IDEAS } from "../mocks.js?v=74";
...
} from "../posts-store.js?v=51";
import { renderPostCard } from "./post-card.js?v=89";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=73";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=281";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=80";
...
import { addMention as addComposerMention } from "../composer-mentions.js?v=45";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=73";
import { open as openImageStudioModal } from "./image-studio/index.js?v=86";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=64";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=51").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=546").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=19").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=24"), import("../folders-store.js?v=19")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=546").then(({ askAngleQuestion }) => {
