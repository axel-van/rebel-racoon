...
import { isSidebarCollapsed, setSidebarCollapsed } from "./sidebar.js?v=59";
import {
...
export function init() {
  ...
  el.addEventListener("click", (event) => {
    ...
    if (clipDraftBtn) {
      ...
      import("../screens/session.js?v=175").then(({ startClipDraftFlow }) => {
    ...
    if (event.target.closest("[data-rpanel-clips-draft]")) {
      ...
      import("../screens/session.js?v=175").then(({ postSubtitleQuestion }) => {
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=175").then(({ askAngleQuestion }) => {
