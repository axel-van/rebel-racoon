...
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=291";
import {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=556").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=556").then(({ askAngleQuestion }) => {
