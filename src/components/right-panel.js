...
import { getThread, subscribe as subscribeThread } from "../assistant.js?v=93";
import { isFlagOn } from "../feature-flags.js?v=19";
import { ideas as MOCK_IDEAS } from "../mocks.js?v=89";
...
} from "../posts-store.js?v=66";
import { renderPostCard } from "./post-card.js?v=105";
import { renderTopPostEcho } from "./top-post-card.js?v=120";
import { renderClipCard } from "./clip-card.js?v=49";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=88";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=353";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=96";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=58";
import { askConnector } from "../connector-ask.js?v=38";
import { renderConnectorLogo } from "../connectors-view.js?v=40";
import { open as openConnectorsModal } from "./connectors-modal.js?v=42";
import { addMention as addComposerMention } from "../composer-mentions.js?v=61";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=89";
import { open as openImageStudioModal } from "./image-studio/index.js?v=104";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=82";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=66").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=618").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=34").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=39"), import("../folders-store.js?v=34")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=618").then(({ askAngleQuestion }) => {
