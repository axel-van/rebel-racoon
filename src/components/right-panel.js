import { html, raw, escapeText, escapeAttr } from "../utils.js?v=1213";
import { getThread, subscribe as subscribeThread } from "../assistant.js?v=1213";
import { isFlagOn } from "../feature-flags.js?v=1213";
import { getPath, navigate } from "../router.js?v=1213";
import { parseHashParams, setHashQuery } from "../url-state.js?v=1213";
import { LANGUAGE_OPTIONS } from "../languages.js?v=1213";
import {
  getPosts,
  removePost,
  insertPost,
...
} from "../posts-store.js?v=1213";
import { renderPostCard } from "./post-card.js?v=1213";
import { renderTopPostEcho } from "./top-post-card.js?v=1213";
import { renderClipCard } from "./clip-card.js?v=1213";
import { onFeedbackClick } from "./feedback-control.js?v=1213";
// Shared compact idea card — same component the standalone Ideas page uses.
import { renderCompactIdeaCard } from "./idea-card-compact.js?v=1213";
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=1213";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=1213";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=1213";
import { open as openRenameModal } from "./rename-modal.js?v=1213";
import { getConnectedConnectors } from "../connectors-store.js?v=1213";
import { getSessionById } from "../sessions-store.js?v=1213";
import { getContextById, getBrandKitGaps } from "../contexts-store.js?v=1213";
import { quickGenerateUrl } from "../image-studio.js?v=1213";
import { askConnector } from "../connector-ask.js?v=1213";
import { renderConnectorLogo } from "../connectors-view.js?v=1213";
import { open as openConnectorsModal } from "./connectors-modal.js?v=1213";
import { addMention as addComposerMention } from "../composer-mentions.js?v=1213";
import { iconFor } from "../file-kinds.js?v=1213";
import { getIdeas, removeIdeasForSources } from "../library.js?v=1213";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=1213";
import { open as openImageStudio } from "./image-studio-v2/index.js?v=1213";
import { open as openConfirmModal } from "./confirm-modal.js?v=1213";
...
export function init() {
  ...
  el.addEventListener("click", (event) => {
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=1213").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session/clip-draft-flow.js?v=1213").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
    ...
    if (reanalyzeBtn) {
      ...
      import("./toast.js?v=1213").then(({ showToast }) =>
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=1213").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=1213"), import("../folders-store.js?v=1213")]).then(
...
function onPostImageRemove(postId) {
  ...
  renderPanel();
  import("./toast.js?v=1213").then(({ showToast }) => showToast("Image removed"));
}
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=1213").then(({ askAngleQuestion }) => {
