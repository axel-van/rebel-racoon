...
import { isSidebarCollapsed, setSidebarCollapsed } from "./sidebar.js?v=54";
import {
...
export function init() {
  ...
  el.addEventListener("click", (event) => {
    ...
    if (clipDraftBtn) {
      ...
      import("../screens/session.js?v=170").then(({ startClipDraftFlow }) => {
    ...
    if (event.target.closest("[data-rpanel-clips-draft]")) {
      ...
      import("../screens/session.js?v=170").then(({ postSubtitleQuestion }) => {
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=170").then(({ askAngleQuestion }) => {
