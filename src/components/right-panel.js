...
import { ideas as MOCK_IDEAS } from "../mocks.js?v=78";
...
} from "../posts-store.js?v=55";
import { renderPostCard } from "./post-card.js?v=93";
import { renderTopPostEcho } from "./top-post-card.js?v=97";
import { renderClipCard } from "./clip-card.js?v=37";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=77";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=302";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=84";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=47";
import { askConnector } from "../connector-ask.js?v=27";
import { renderConnectorLogo } from "../connectors-view.js?v=29";
import { open as openConnectorsModal } from "./connectors-modal.js?v=30";
import { addMention as addComposerMention } from "../composer-mentions.js?v=49";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=77";
import { open as openImageStudioModal } from "./image-studio/index.js?v=90";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=68";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=55").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=567").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=23").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=28"), import("../folders-store.js?v=23")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=567").then(({ askAngleQuestion }) => {
