...
import { getThread, subscribe as subscribeThread } from "../assistant.js?v=86";
import { isFlagOn } from "../feature-flags.js?v=19";
import { ideas as MOCK_IDEAS } from "../mocks.js?v=82";
...
} from "../posts-store.js?v=59";
import { renderPostCard } from "./post-card.js?v=98";
import { renderTopPostEcho } from "./top-post-card.js?v=106";
import { renderClipCard } from "./clip-card.js?v=42";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=81";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=323";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=89";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=51";
import { askConnector } from "../connector-ask.js?v=31";
import { renderConnectorLogo } from "../connectors-view.js?v=33";
import { open as openConnectorsModal } from "./connectors-modal.js?v=35";
import { addMention as addComposerMention } from "../composer-mentions.js?v=54";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=82";
import { open as openImageStudioModal } from "./image-studio/index.js?v=96";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=74";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=59").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=588").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=27").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=32"), import("../folders-store.js?v=27")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=588").then(({ askAngleQuestion }) => {
