...
import { renderTopPostPreview } from "./top-post-card.js?v=6";
...
import { isSidebarCollapsed, setSidebarCollapsed } from "./sidebar.js?v=97";
import {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=297").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=297").then(({ askAngleQuestion }) => {
