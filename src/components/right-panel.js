...
import { ideas as MOCK_IDEAS } from "../mocks.js?v=58";
...
} from "../posts-store.js?v=38";
import { renderPostCard } from "./post-card.js?v=71";
import { renderTopPostEcho } from "./top-post-card.js?v=68";
import { renderClipCard } from "./clip-card.js?v=16";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=56";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=192";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=63";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=30";
import { askConnector } from "../connector-ask.js?v=10";
import { renderConnectorLogo } from "../connectors-view.js?v=12";
import { open as openConnectorsModal } from "./connectors-modal.js?v=13";
import { addMention as addComposerMention } from "../composer-mentions.js?v=25";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=56";
import { open as openImageStudioModal } from "./image-studio/index.js?v=44";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=7";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=38").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=444").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=11").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=11"), import("../folders-store.js?v=6")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=444").then(({ askAngleQuestion }) => {
