...
import { getThread, subscribe as subscribeThread } from "../assistant.js?v=58";
import { isFlagOn } from "../feature-flags.js?v=10";
import { ideas as MOCK_IDEAS } from "../mocks.js?v=56";
...
} from "../posts-store.js?v=36";
import { renderPostCard } from "./post-card.js?v=68";
import { renderTopPostEcho } from "./top-post-card.js?v=66";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=51";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=175";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=61";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=28";
import { askConnector } from "../connector-ask.js?v=8";
import { renderConnectorLogo } from "../connectors-view.js?v=10";
import { open as openConnectorsModal } from "./connectors-modal.js?v=11";
import { addMention as addComposerMention } from "../composer-mentions.js?v=22";
...
import { open as openImageStudioModal } from "./image-studio/index.js?v=28";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=36").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=427").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=9").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=9"), import("../folders-store.js?v=4")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=427").then(({ askAngleQuestion }) => {
