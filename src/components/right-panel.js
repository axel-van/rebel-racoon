...
import { getThread, subscribe as subscribeThread } from "../assistant.js?v=89";
import { isFlagOn } from "../feature-flags.js?v=19";
import { ideas as MOCK_IDEAS } from "../mocks.js?v=85";
...
} from "../posts-store.js?v=62";
import { renderPostCard } from "./post-card.js?v=101";
import { renderTopPostEcho } from "./top-post-card.js?v=112";
import { renderClipCard } from "./clip-card.js?v=45";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=84";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=335";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=92";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=54";
import { askConnector } from "../connector-ask.js?v=34";
import { renderConnectorLogo } from "../connectors-view.js?v=36";
import { open as openConnectorsModal } from "./connectors-modal.js?v=38";
import { addMention as addComposerMention } from "../composer-mentions.js?v=57";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=85";
import { open as openImageStudioModal } from "./image-studio/index.js?v=100";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=78";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=62").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=600").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=30").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=35"), import("../folders-store.js?v=30")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=600").then(({ askAngleQuestion }) => {
