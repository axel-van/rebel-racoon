...
import { isSidebarCollapsed, setSidebarCollapsed } from "./sidebar.js?v=55";
import {
...
export function init() {
  ...
  el.addEventListener("click", (event) => {
    ...
    if (clipDraftBtn) {
      ...
      import("../screens/session.js?v=171").then(({ startClipDraftFlow }) => {
    ...
    if (event.target.closest("[data-rpanel-clips-draft]")) {
      ...
      import("../screens/session.js?v=171").then(({ postSubtitleQuestion }) => {
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=171").then(({ askAngleQuestion }) => {
