...
import { renderTopPostPreview } from "./top-post-card.js?v=5";
...
import { isSidebarCollapsed, setSidebarCollapsed } from "./sidebar.js?v=96";
import {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=296").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=296").then(({ askAngleQuestion }) => {
