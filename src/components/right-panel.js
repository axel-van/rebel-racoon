...
import { ideas as MOCK_IDEAS } from "../mocks.js?v=57";
...
} from "../posts-store.js?v=37";
import { renderPostCard } from "./post-card.js?v=69";
import { renderTopPostEcho } from "./top-post-card.js?v=67";
import { renderClipCard } from "./clip-card.js?v=14";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=53";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=186";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=62";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=29";
import { askConnector } from "../connector-ask.js?v=9";
import { renderConnectorLogo } from "../connectors-view.js?v=11";
import { open as openConnectorsModal } from "./connectors-modal.js?v=12";
import { addMention as addComposerMention } from "../composer-mentions.js?v=23";
...
import { open as openImageStudioModal } from "./image-studio/index.js?v=42";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=37").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=438").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=10").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=10"), import("../folders-store.js?v=5")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=438").then(({ askAngleQuestion }) => {
