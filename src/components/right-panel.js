...
import { getThread, subscribe as subscribeThread } from "../assistant.js?v=48";
...
import { isSidebarCollapsed, setSidebarCollapsed } from "./sidebar.js?v=113";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=50";
...
import { addMention as addComposerMention } from "../composer-mentions.js?v=12";
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=313").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=313").then(({ askAngleQuestion }) => {
