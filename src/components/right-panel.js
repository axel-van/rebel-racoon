...
import { getThread, subscribe as subscribeThread } from "../assistant.js?v=104";
import { isFlagOn } from "../feature-flags.js?v=23";
import { ideas as MOCK_IDEAS } from "../mocks.js?v=96";
...
} from "../posts-store.js?v=74";
import { renderPostCard } from "./post-card.js?v=116";
import { renderTopPostEcho } from "./top-post-card.js?v=141";
import { renderClipCard } from "./clip-card.js?v=60";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=98";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=442";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=107";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=66";
import { askConnector } from "../connector-ask.js?v=46";
import { renderConnectorLogo } from "../connectors-view.js?v=48";
import { open as openConnectorsModal } from "./connectors-modal.js?v=50";
import { addMention as addComposerMention } from "../composer-mentions.js?v=72";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=100";
import { open as openImageStudioModal } from "./image-studio/index.js?v=116";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=94";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=74").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=707").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=42").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=47"), import("../folders-store.js?v=42")]).then(
...
function onPostImageRemove(postId) {
  ...
  renderPanel();
  import("./toast.js?v=21").then(({ showToast }) => showToast("Image removed"));
}
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=707").then(({ askAngleQuestion }) => {
