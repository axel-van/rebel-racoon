...
import { getThread, subscribe as subscribeThread } from "../assistant.js?v=76";
import { isFlagOn } from "../feature-flags.js?v=18";
import { ideas as MOCK_IDEAS } from "../mocks.js?v=73";
...
import { renderTopPostEcho } from "./top-post-card.js?v=87";
import { renderClipCard } from "./clip-card.js?v=32";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=72";
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=79";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=42";
import { askConnector } from "../connector-ask.js?v=22";
import { renderConnectorLogo } from "../connectors-view.js?v=24";
import { open as openConnectorsModal } from "./connectors-modal.js?v=25";
import { addMention as addComposerMention } from "../composer-mentions.js?v=44";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=72";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=45").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=508").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=18").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=18"), import("../folders-store.js?v=13")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=508").then(({ askAngleQuestion }) => {
