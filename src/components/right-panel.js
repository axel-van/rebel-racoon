...
import { getThread, subscribe as subscribeThread } from "../assistant.js?v=100";
import { isFlagOn } from "../feature-flags.js?v=23";
import { ideas as MOCK_IDEAS } from "../mocks.js?v=92";
...
} from "../posts-store.js?v=70";
import { renderPostCard } from "./post-card.js?v=112";
import { renderTopPostEcho } from "./top-post-card.js?v=133";
import { renderClipCard } from "./clip-card.js?v=56";
...
import { open as openVideoClipsModal } from "./video-clips-modal.js?v=94";
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=413";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=103";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=62";
import { askConnector } from "../connector-ask.js?v=42";
import { renderConnectorLogo } from "../connectors-view.js?v=44";
import { open as openConnectorsModal } from "./connectors-modal.js?v=46";
import { addMention as addComposerMention } from "../composer-mentions.js?v=68";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=96";
import { open as openImageStudioModal } from "./image-studio/index.js?v=112";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=90";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=70").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=678").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=38").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=43"), import("../folders-store.js?v=38")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=678").then(({ askAngleQuestion }) => {
