...
import { getThread, subscribe as subscribeThread } from "../assistant.js?v=51";
...
import { isSidebarCollapsed, setSidebarCollapsed } from "./sidebar.js?v=128";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=53";
...
import { addMention as addComposerMention } from "../composer-mentions.js?v=15";
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=328").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=328").then(({ askAngleQuestion }) => {
