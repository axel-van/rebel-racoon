...
import { ideas as MOCK_IDEAS } from "../mocks.js?v=91";
...
} from "../posts-store.js?v=68";
import { renderPostCard } from "./post-card.js?v=109";
import { renderTopPostEcho } from "./top-post-card.js?v=128";
import { renderClipCard } from "./clip-card.js?v=53";
...
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=396";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=100";
import { open as openRenameModal } from "./rename-modal.js?v=2";
import { getConnectedConnectors } from "../connectors-store.js?v=60";
import { askConnector } from "../connector-ask.js?v=40";
import { renderConnectorLogo } from "../connectors-view.js?v=42";
import { open as openConnectorsModal } from "./connectors-modal.js?v=44";
import { addMention as addComposerMention } from "../composer-mentions.js?v=65";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=93";
import { open as openImageStudioModal } from "./image-studio/index.js?v=108";
import { open as openImageStudioV2Modal } from "./image-studio-v2/index.js?v=86";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=68").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=661").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onPostRewrite(postId, intent = "fresh") {
  ...
  import("../draft-rewrite.js?v=36").then(({ startRewrite }) => {
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=41"), import("../folders-store.js?v=36")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=661").then(({ askAngleQuestion }) => {
