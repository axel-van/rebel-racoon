...
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=132";
import {
...
import { open as openScheduleModal } from "./schedule-modal.js?v=47";
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=331").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=331").then(({ askAngleQuestion }) => {
