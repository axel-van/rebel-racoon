...
import { escapeHtml } from "../utils.js?v=1167";
import { requestOpen, notifyClose } from "../modal-coordinator.js?v=1167";
import { FORMATS, NETWORK_FORMATS, CLIP_RATIO_ORDER, ratioNetworksMeta, ratioValue } from "../clip-formats.js?v=1167";
import { iconFor } from "../file-kinds.js?v=1167";
import { DEFAULT_PRESET, buildCaptions, videoForClip } from "../clip-captions.js?v=1167";
...
function renderOptions() {
  ...
  if (editorTab === "subtitles") {
    import("../caption-editor.js?v=1167").then(({ refreshControls }) => refreshControls());
  }
}
...
function syncCaptionMount() {
  ...
  import("../caption-editor.js?v=1167").then((mod) => {
    capMod = mod;
    if (want) {
...
function close() {
  ...
  if (captionMounted) {
    import("../caption-editor.js?v=1167").then(({ unmount }) => unmount());
    captionMounted = false;
  }
