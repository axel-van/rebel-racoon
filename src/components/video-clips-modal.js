...
import { escapeHtml } from "../utils.js?v=1217";
import { requestOpen, notifyClose } from "../modal-coordinator.js?v=1217";
import { FORMATS, NETWORK_FORMATS, CLIP_RATIO_ORDER, ratioNetworksMeta, ratioValue } from "../clip-formats.js?v=1217";
import { iconFor } from "../file-kinds.js?v=1217";
import { DEFAULT_PRESET, buildCaptions, videoForClip } from "../clip-captions.js?v=1217";
...
function renderOptions() {
  ...
  if (editorTab === "subtitles") {
    import("../caption-editor.js?v=1217").then(({ refreshControls }) => refreshControls());
  }
}
...
function syncCaptionMount() {
  ...
  import("../caption-editor.js?v=1217").then((mod) => {
    capMod = mod;
    if (want) {
...
function close() {
  ...
  if (captionMounted) {
    import("../caption-editor.js?v=1217").then(({ unmount }) => unmount());
    captionMounted = false;
  }
