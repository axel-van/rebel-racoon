...
export const contexts = [
  {
    ...
    // Research config — which sources Archie scans for this Playbook, how often,
    // and whether arrivals are announced. See research-catalog.js. A Playbook
    // without this field falls back to the catalog defaults on read.
    research: {
  ...
  },
  {
    ...
    research: {
      enabledSourceIds: ["influencer-posts"],
      cadence: "monthly",
      notify: false,
    },
    businessSummary:
...
// ── Research findings ──────────────────────────────────────────────────────
//
// A FINDING is an evidence-backed research insight: a headline, a short
// summary, a long-form synthesis, the source posts it was derived from, and
// the idea seeds it can become. Findings sit upstream of Ideas in the
// pipeline: Source → Finding → Idea → Draft → Schedule.
//
// Each finding is tagged with the Playbook it was produced for (`contextId`)
// and the research source that produced it (`sourceId`, see
// research-catalog.js). `dedupeKey` is stable across scans — dismissal memory
// keys on it, not on `id`, so a rejected finding never comes back even if a
// later scan re-derives the same insight under a new id.
//
// `ideaSeeds` are pre-authored (never generated) so "Turn into ideas" produces
// ideas that genuinely read as derived from THAT finding — same approach as
// library.js SCRIPTS and anglesByIdea. injectIdeasForSource fills every other
// field, so the seeds only carry what's specific.
//
// `researchFindings` is what the feed already holds. `researchScanPool` is
// held back so "Run a scan" has something real to deliver — and most of it
// comes from sources that are OFF by default, which is what makes the Sources
// toggles observably load-bearing (runScan skips disabled sources).

export const researchFindings = [
  {
    id: "rf-opinions-over-features",
    dedupeKey: "competitors-lead-with-opinions",
    contextId: "ctx-acme",
    sourceId: "competitor-posts",
    researchType: "Competitive intelligence",
    headline: "Your competitors stopped announcing features and started publishing opinions",
    summary:
      "Notion, Linear and Basecamp's ten best posts last month were all takes on how teams should work. Not one was a feature announcement.",
    synthesis: [
      "A scan of Notion, Linear and Basecamp's last 30 days shows the same shift in all three feeds: the posts they put budget and founder time behind are arguments, not announcements. Release notes still go out, but they sit at the bottom of the engagement range — polite reactions from existing users, almost no reshares outside the follower graph.",
      "What travels instead is a position: how many people should be in a standup, why a roadmap should be a paragraph rather than a board, when a process is worth the meeting it costs. These posts get argued with in the comments, and the argument is what carries them into feeds that don't already follow the brand.",
      "The opening this leaves is specific. All three are making the case for how work should happen in general; none of them is making it about the 50–200-person B2B company that has outgrown its first process and not yet hired the ops person to own the second one. That audience is exactly yours, and the argument is currently unclaimed.",
    ],
    posts: [
      {
        id: "rf1-p1",
        network: "x",
        publishedOn: "Jul 21, 2026",
        author: { name: "Linear", handle: "@linear", initials: "LI", accent: "purple" },
        text: "A roadmap that needs a legend isn't a roadmap. If you can't say what you're building this quarter in one paragraph, the board is doing the thinking for you.",
        likes: 4820,
        comments: 312,
        reposts: 690,
      },
      {
        id: "rf1-p2",
        network: "linkedin",
        publishedOn: "Jul 17, 2026",
        author: { name: "Notion", handle: "Notion", initials: "NO", accent: "grey" },
        text: "Most teams don't have a documentation problem. They have a decisions problem — nobody wrote down why, so every quarter re-litigates the same trade-off.",
        likes: 2140,
        comments: 188,
        reposts: 274,
      },
      {
        id: "rf1-p3",
        network: "x",
        publishedOn: "Jul 12, 2026",
        author: { name: "Basecamp", handle: "@basecamp", initials: "BC", accent: "yellow" },
        text: "Six-week cycles, then two weeks of nothing scheduled. The cooldown isn't a break from the work. It's where the next six weeks get decided.",
        likes: 3050,
        comments: 241,
        reposts: 512,
      },
    ...
    ideaSeeds: [
      {
        title: "The second process nobody owns",
        body: "At 50 people your first process breaks and there's no ops hire yet. Name that gap, then show the three decisions that unblock it.",
        kind: "insight",
        rationale:
          "Your competitors argue about how work should happen in general. This scopes the argument to the company size you actually sell to, which none of them claim.",
        channels: ["linkedin"],
      },
      {
        title: "Release notes nobody read",
        body: "Open on the number: how much of last quarter's changelog your customers could name. Then make the case for shipping opinions alongside features.",
        kind: "hook",
        rationale:
          "Turns the finding itself into the post — a contrarian beat backed by the same pattern the scan surfaced.",
        channels: ["linkedin", "x"],
      },
      {
        title: "One paragraph, not a board",
        body: "Take Linear's roadmap argument and answer it from the operator's side: what a one-paragraph roadmap costs the person who has to staff it.",
        kind: "story",
        rationale:
          "Replies to the highest-performing post in the set rather than restating it, which is what earns reach outside your own follower graph.",
        channels: ["linkedin"],
      },
    ],
    scannedAt: "5h ago",
  ...
  {
    id: "rf-workflow-over-specs",
    dedupeKey: "specs-lose-to-workflow-teardowns",
    contextId: "ctx-acme",
    sourceId: "competitor-posts",
    researchType: "Competitive intelligence",
    headline: "Rivals lead with what their tool does; the posts that spread show a real team's week",
    summary:
      "Spec-led launch posts in your category earn polite engagement. The ones that travel are teardowns of an actual workflow, where the tool is incidental.",
    synthesis: [
      "Across the category, launch posts follow a template: the feature name, three bullets of capability, a screenshot of the empty state. Engagement on these is narrow and technical — existing users asking about edge cases, competitors' employees arguing about parity.",
      "The posts that break out of that pattern are structurally different. They walk through one team's actual week: what came in, who decided what, where the handoff failed, what changed after. The product appears, but as the thing that happened to be open on screen — never as the subject.",
      "The gap is framing, not capability. Where the category presents a workflow as a feature list, the posts that spread present the same workflow as a week someone lived through. That's the register your Playbook already asks for — concrete time saved, customer outcomes, no feature lists — and the category is largely leaving it open.",
    ],
    posts: [
      {
        id: "rf2-p1",
        network: "linkedin",
        publishedOn: "Jul 19, 2026",
        author: { name: "Notion", handle: "Notion", initials: "NO", accent: "grey" },
        text: "Introducing linked database views: filter once, reuse everywhere, sync across teamspaces. Rollups now support nested properties, and templates inherit permissions.",
        likes: 410,
        comments: 96,
        reposts: 38,
      },
      {
        ...
        author: { name: "Linear", handle: "@linear", initials: "LI", accent: "purple" },
        text: "Sub-issues, cycle capacity, and triage rules now work together. Configure once at the team level and every project inherits it.",
        likes: 620,
        comments: 74,
        reposts: 61,
      },
      {
        id: "rf2-p3",
        network: "linkedin",
        publishedOn: "Jul 8, 2026",
        author: { name: "Priya Raman", handle: "Priya Raman", initials: "PR", accent: "green" },
        text: "Our Monday used to take four hours: three tools, two spreadsheets, one meeting where nobody decided anything. Here's what we cut, in order, and what it cost us to be wrong about the first two.",
        likes: 5410,
        comments: 402,
        reposts: 880,
      },
    ...
    ideaSeeds: [
      {
        title: "One customer's Monday, before and after",
        body: "Pick a customer, reconstruct their Monday in four hours of detail, then show the same Monday after. Numbers, not adjectives.",
        kind: "story",
        rationale:
          "The one post in the set that outperformed everything was exactly this shape — a lived week, product incidental.",
        channels: ["linkedin"],
      },
      {
        title: "What we cut, in order",
        body: "The sequence matters more than the list: what went first, what turned out to be load-bearing, what you put back.",
        kind: "insight",
        rationale:
          "Admitting the two wrong calls is what made the reference post credible; the same structure works from the vendor side.",
        channels: ["linkedin", "x"],
      },
    ],
    scannedAt: "1d ago",
  ...
  {
    id: "rf-stack-teardowns",
    dedupeKey: "creators-win-with-stack-teardowns",
    contextId: "ctx-acme",
    sourceId: "influencer-posts",
    researchType: "Creator signals",
    headline: "Ops creators are winning with unglamorous “here is my actual stack” teardowns",
    summary:
      "The three creators your audience follows most each posted a full stack breakdown this month, and each one beat their own median by roughly 3×.",
    synthesis: [
      "The pattern is consistent enough to plan against. Each of these creators normally posts advice; each posted one inventory this month — every tool, what it costs, what it replaced, what they'd drop tomorrow — and each of those inventories outperformed their median post by around three times.",
      "What makes them work is the unflattering detail. Two of the three name a tool they pay for and don't use. One publishes the monthly total. The credibility comes from the admission, not from the recommendation, which is why the format resists the usual sponsored-content discount.",
      "For a brand this format is available but only from a specific angle: you can't publish your own stack credibly, but you can publish a customer's, or the stack of the team that built the thing you sell. The tool being one line in someone else's honest inventory is worth more than a paragraph of your own copy.",
    ],
    posts: [
      {
        id: "rf3-p1",
        network: "linkedin",
        publishedOn: "Jul 24, 2026",
        author: { name: "Dana Whitfield", handle: "Dana Whitfield", initials: "DW", accent: "red" },
        text: "Every tool our 80-person team pays for, what it replaced, and the two I'd cancel today if renewal weren't in March. Total: $4,180 a month.",
        likes: 7900,
        comments: 640,
        reposts: 1240,
      },
      {
        id: "rf3-p2",
        network: "x",
        publishedOn: "Jul 20, 2026",
        author: { name: "Marcus Oyelaran", handle: "@marcus_ops", initials: "MO", accent: "orange" },
        text: "I've been paying for a project tool I open twice a month. Keeping it because the migration is worse than the invoice is a real reason and nobody says it out loud.",
        likes: 4300,
        comments: 388,
        reposts: 705,
      },
      {
        id: "rf3-p3",
        network: "linkedin",
        publishedOn: "Jul 11, 2026",
        author: { name: "Sofia Brandt", handle: "Sofia Brandt", initials: "SB", accent: "electric-blue" },
        text: "Our stack, minus the parts that make it look considered. Four tools do 90% of the work. The other nine exist because someone left and nobody cancelled.",
        likes: 6120,
        comments: 511,
        reposts: 980,
      },
    ...
    ideaSeeds: [
      {
        title: "A customer's stack, including what they'd cancel",
        body: "Publish one customer's full inventory with the monthly total and the tools they regret — yours included if it's honest.",
        kind: "story",
        rationale:
          "The format only works with the unflattering detail; borrowing a customer's inventory is the one way a vendor can carry it.",
        channels: ["linkedin"],
      },
      {
        title: "The tool nobody cancels",
        body: "Name the real reason teams keep software they don't open: migration costs more than the invoice. Then say what that implies about switching to you.",
        kind: "hook",
        rationale:
          "Marcus's line was the highest-engagement moment in the set and it's an objection your sales team already hears.",
        channels: ["linkedin", "x"],
      },
    ],
    scannedAt: "3h ago",
  ...
  {
    id: "rf-flat-pricing-comments",
    dedupeKey: "basecamp-flat-pricing-pulls-your-buyers",
    contextId: "ctx-acme",
    sourceId: "competitor-posts",
    researchType: "Competitive intelligence",
    headline: "Basecamp's flat-pricing posts are collecting your buyers in the comments",
    summary:
      "Three pricing posts this quarter drew comment threads full of ops leads at your exact company size doing per-seat maths out loud.",
    synthesis: [
      "Basecamp posts about flat pricing roughly monthly, and the posts themselves perform unremarkably. The comments are the asset: each thread fills with people naming their headcount and computing what per-seat pricing costs them at that size.",
      "The commenters are your buyers. Titles cluster around operations and marketing leadership; stated headcounts cluster between 40 and 180. They are not asking whether flat pricing is better — they are asking each other how to justify a per-seat renewal internally.",
      "Nobody in those threads is answering the question they're actually asking, which is how to defend the line item, not how to lower it. That's a post, and it doesn't require matching anyone's pricing model.",
    ],
    posts: [
      {
        id: "rf4-p1",
        network: "x",
        publishedOn: "Jul 2, 2026",
        author: { name: "Basecamp", handle: "@basecamp", initials: "BC", accent: "yellow" },
        text: "One price. Everyone in the company. No per-seat maths, no negotiating with your own finance team about who deserves a login.",
        likes: 2890,
        comments: 470,
        reposts: 388,
      },
      {
        ...
        network: "x",
        publishedOn: "Jul 2, 2026",
        author: { name: "Tomás Herrera", handle: "@tomasops", initials: "TH", accent: "menthol" },
        text: "We're 140 people and I spend a week each renewal proving which 40 seats are load-bearing. That week costs more than the seats.",
        likes: 1210,
        comments: 96,
        reposts: 143,
      },
    ...
    ideaSeeds: [
      {
        title: "How to defend the line item",
        body: "Not a pricing argument — a renewal argument. Give the ops lead the three numbers that make the seat count defensible to finance.",
        kind: "insight",
        rationale:
          "The comment threads show buyers asking how to justify the renewal, not how to pay less. Nobody is answering that.",
        channels: ["linkedin"],
      },
      {
        title: "The week before renewal",
        body: "Open on Tomás's number: a week of proving which seats matter costs more than the seats. Then show what removes the week.",
        kind: "stat",
        rationale: "A buyer's own sentence, already validated in public, doing the work of the hook.",
        channels: ["linkedin", "x"],
      },
    ],
    scannedAt: "6d ago",
  ...
  {
    id: "rf-founder-reversals",
    dedupeKey: "founder-reversal-posts-outperform",
    contextId: "ctx-founder-voice",
    sourceId: "influencer-posts",
    researchType: "Creator signals",
    headline: "Founder posts that admit a reversal are outperforming the confident ones",
    summary:
      "Across the founders your audience follows, the best-performing post of the month was a changed mind — not a lesson, prediction or framework.",
    synthesis: [
      "The founders in this set post advice at a steady rate and a reversal rarely. When they do, the reversal is the month's outlier: three to five times their median reach, and a comment section that argues with the substance rather than congratulating the author.",
      "The mechanics are specific. The strongest ones name the position they held, when they held it, what it cost, and what changed their mind — in that order, without softening the original claim. The ones that underperform hedge the earlier position, which reads as a framework rather than an admission.",
      "This is directly usable: the Playbook already asks for sharp opinions and lived experience with no promotional content. A reversal is both at once, and it's the one shape that can't be written by someone who wasn't there.",
    ],
    posts: [
      {
        id: "rf5-p1",
        network: "linkedin",
        publishedOn: "Jul 22, 2026",
        author: { name: "Elena Fisk", handle: "Elena Fisk", initials: "EF", accent: "purple" },
        text: "I spent two years telling founders to hire a Head of Ops before 50 people. We did, twice, and both times the role was invented before the problem. Here's what I'd do instead.",
        likes: 8400,
        comments: 712,
        reposts: 1420,
      },
      {
        id: "rf5-p2",
        network: "x",
        publishedOn: "Jul 14, 2026",
        author: { name: "Ravi Chaudhary", handle: "@ravibuilds", initials: "RC", accent: "green" },
        text: "Async-only was the wrong hill. It worked for 12 people and quietly broke at 40 — the meetings came back as documents nobody read.",
        likes: 3960,
        comments: 344,
        reposts: 588,
      },
    ...
    ideaSeeds: [
      {
        title: "The role we invented before the problem",
        body: "Name the hire you made too early, when you made it, and what it cost. No hedging on the original reasoning.",
        kind: "story",
        rationale:
          "The reversals that outperform state the old position plainly; the ones that hedge read as frameworks and underperform.",
        channels: ["linkedin"],
      },
      {
        title: "What broke at 40 people",
        body: "One practice that worked at 12 and failed at 40, and the specific week you noticed.",
        kind: "insight",
        rationale: "Matches the second-strongest post in the set and lands squarely in lived experience.",
        channels: ["linkedin", "x"],
      },
    ],
    scannedAt: "2d ago",
...
export const researchScanPool = [
  {
    id: "rf-pricing-bundles",
    dedupeKey: "rivals-move-to-seat-bundles",
    contextId: "ctx-acme",
    sourceId: "competitor-posts",
    researchType: "Competitive intelligence",
    headline: "Two competitors quietly moved from per-seat to seat bundles this month",
    summary:
      "Neither announced it. Both pricing pages now sell blocks of seats, and the change showed up first in their sales replies.",
    synthesis: [
      "Two of the three competitors in your Playbook changed pricing structure in the last three weeks without a post about it. Per-seat lines became bundles — blocks of 25 and 50 — and the per-user number disappeared from the page entirely.",
      "The change surfaced publicly through replies rather than announcements: sales conversations in threads now quote a bundle, and a handful of customers have posted screenshots comparing their old invoice to the new one.",
      "Undiscussed pricing changes are an opening for whoever explains them first. The buyers doing the comparison in public are asking a straightforward question — what does this cost a team that grows by six people a quarter — and no vendor is answering it.",
    ],
    posts: [
      {
        id: "rfp1-p1",
        network: "x",
        publishedOn: "Jul 25, 2026",
        author: { name: "Hannah Kwon", handle: "@hannahk", initials: "HK", accent: "orange" },
        text: "Renewal quote came back as a 50-seat block. We're 31 people. Asked what happens at 51 and got a meeting invite instead of a number.",
        likes: 2210,
        comments: 214,
        reposts: 336,
      },
      {
        id: "rfp1-p2",
        network: "linkedin",
        publishedOn: "Jul 23, 2026",
        author: { name: "Ops Weekly", handle: "Ops Weekly", initials: "OW", accent: "soft-blue" },
        text: "Two of the big four dropped per-user pricing from their pages this month. Nobody sent an email about it.",
        likes: 1480,
        comments: 132,
        reposts: 290,
      },
    ...
    ideaSeeds: [
      {
        title: "What it costs to grow by six people",
        body: "Answer the question the comparison threads keep asking: the real cost curve for a team adding six people a quarter.",
        kind: "stat",
        rationale: "Buyers are doing this maths in public and no vendor has published the answer.",
        channels: ["linkedin"],
      },
      {
        title: "The pricing change nobody announced",
        body: "Explain the shift to bundles plainly, including why vendors make it. Being the one who explains it is the position.",
        kind: "insight",
        rationale: "Undiscussed changes reward whoever narrates them first, and it needs no competitive claim.",
        channels: ["linkedin", "x"],
      },
    ],
    scannedAt: "just now",
  ...
  {
    id: "rf-one-chart-posts",
    dedupeKey: "creators-drop-carousels-for-one-chart",
    contextId: "ctx-acme",
    sourceId: "influencer-posts",
    researchType: "Creator signals",
    headline: "Creators are dropping carousels for single-chart posts",
    summary:
      "The ops creators your audience follows cut carousel output by half this quarter and replaced it with one chart and two sentences.",
    synthesis: [
      "Carousel volume among these creators is down sharply this quarter. What replaced it is smaller: one chart, a title that states the finding, and two sentences of interpretation. No cover slide, no summary slide, no call to swipe.",
      "The engagement pattern explains the switch. Single-chart posts hold roughly the same reach as a good carousel but earn noticeably more saves, and the comments quote the number rather than thanking the author for the breakdown.",
      "It's a cheap format to adopt and it suits evidence-led content: one finding, visible in a glance, argued in two lines.",
    ],
    posts: [
      {
        id: "rfp2-p1",
        ...
        publishedOn: "Jul 24, 2026",
        author: { name: "Dana Whitfield", handle: "Dana Whitfield", initials: "DW", accent: "red" },
        text: "One chart: time-to-first-value against retention at 12 months, 400 B2B teams. The correlation is embarrassing and it's the only onboarding metric I'd defend.",
        likes: 5300,
        comments: 288,
        reposts: 910,
      },
      {
        id: "rfp2-p2",
        network: "linkedin",
        publishedOn: "Jul 16, 2026",
        author: { name: "Sofia Brandt", handle: "Sofia Brandt", initials: "SB", accent: "electric-blue" },
        text: "Stopped making carousels in April. Same reach, twice the saves, a tenth of the work.",
        likes: 4110,
        comments: 356,
        reposts: 620,
      },
    ...
    ideaSeeds: [
      {
        title: "One chart, two sentences",
        body: "Take the single most defensible number you have and publish it alone. No cover slide, no swipe prompt.",
        kind: "stat",
        rationale: "Same reach as a carousel for a tenth of the production, with more saves.",
        channels: ["linkedin"],
      },
    ],
    scannedAt: "just now",
  ...
  {
    id: "rf-onboarding-week",
    dedupeKey: "onboarding-costs-happy-customers-a-week",
    contextId: "ctx-acme",
    sourceId: "brand-feedback",
    researchType: "Audience feedback",
    headline: "Your happiest customers still lose a week to setup",
    summary:
      "Eleven positive reviews this quarter open with praise and then mention the same first week. It is the only complaint that survives a five-star rating.",
    synthesis: [
      "Sorting this quarter's inbound by sentiment puts a specific pattern in the positive pile: eleven separate reviews and comments that recommend the product and then describe the first week as the part they'd warn a friend about.",
      "The detail repeats. Import works, permissions don't; the second person to join has to redo the first person's configuration; nobody finds the template that would have saved the whole afternoon. None of it is a bug report, which is why it never reached the roadmap.",
      "A complaint that survives a five-star review is the most credible kind you get. Saying it out loud before a prospect discovers it converts a liability into a reason to trust the rest.",
    ],
    posts: [
      {
        id: "rfp3-p1",
        network: "linkedin",
        publishedOn: "Jul 18, 2026",
        author: { name: "Grace Adeyemi", handle: "Grace Adeyemi", initials: "GA", accent: "mermaid" },
        text: "Genuinely recommend it — we've cut two meetings a week. Would have cut three if the first week hadn't gone on setting up permissions twice.",
        likes: 890,
        comments: 74,
        reposts: 61,
      },
      {
        id: "rfp3-p2",
        network: "x",
        publishedOn: "Jul 9, 2026",
        author: { name: "Ben Osei", handle: "@benosei", initials: "BO", accent: "green" },
        text: "Six months in and happy. Still think about the afternoon I spent building a board that turned out to be a template.",
        likes: 640,
        comments: 52,
        reposts: 38,
      },
    ...
    ideaSeeds: [
      {
        title: "The week we're not proud of",
        body: "Say the setup week out loud, name the three things that go wrong in it, and show what changes this quarter.",
        kind: "story",
        rationale:
          "A complaint that survives a five-star review is the most credible material you have; saying it first defuses it.",
        channels: ["linkedin"],
      },
      {
        title: "Permissions, twice",
        body: "The second person to join redoes the first person's configuration. One specific failure, one specific fix.",
        kind: "insight",
        rationale: "The most repeated detail across eleven reviews, and concrete enough to be checkable.",
        channels: ["linkedin"],
      },
    ],
    scannedAt: "just now",
  ...
  {
    id: "rf-permission-model-gap",
    dedupeKey: "rivals-users-want-permission-model",
    contextId: "ctx-acme",
    sourceId: "competitor-monitoring",
    researchType: "Competitive intelligence",
    headline: "Competitors' users keep asking for the permission model you already ship",
    summary:
      "Guest access with scoped visibility is the most-requested item in two rivals' public feedback boards. It has been in your product since March.",
    synthesis: [
      "Two competitors run public feedback boards, and the top open request on both is the same: let an outside collaborator see one project without seeing the workspace. Both threads run to hundreds of votes and years of comments.",
      "The requesters describe the same job — an agency, a contractor, a client who needs one view — and the same workaround, which is a duplicate workspace kept in sync by hand.",
      "You shipped this in March and have never made an argument out of it. The audience is assembled, public, and describing the problem in its own words.",
    ],
    posts: [
      {
        id: "rfp4-p1",
        network: "x",
        publishedOn: "Jul 20, 2026",
        author: { name: "Maya Lindqvist", handle: "@mayalind", initials: "ML", accent: "purple" },
        text: "Three years on this feature request. We now run a second workspace for clients and sync it by hand every Friday.",
        likes: 1840,
        comments: 226,
        reposts: 402,
      },
      {
        id: "rfp4-p2",
        network: "x",
        publishedOn: "Jul 6, 2026",
        author: { name: "Owen Baptiste", handle: "@obaptiste", initials: "OB", accent: "orange" },
        text: "All I want is for a contractor to see one project. One. Not the roadmap, not salaries in a database three clicks away.",
        likes: 1120,
        comments: 148,
        reposts: 205,
      },
    ...
    ideaSeeds: [
      {
        title: "One project, nothing else",
        body: "Show scoped guest access doing the exact job those threads describe: a contractor who sees one project and nothing adjacent.",
        kind: "insight",
        rationale: "The demand is public, specific, and years old; you already ship the answer and have never said so.",
        channels: ["linkedin"],
      },
      {
        title: "The workspace you sync by hand",
        body: "Open on the Friday ritual — a duplicate workspace kept in sync manually — then remove it.",
        kind: "hook",
        rationale: "A workaround stated in a buyer's own words, already validated by 226 comments.",
        channels: ["linkedin", "x"],
      },
    ],
    scannedAt: "just now",
  ...
  {
    id: "rf-ai-teammate-language",
    dedupeKey: "ai-teammate-replaces-ai-assistant",
    contextId: "ctx-acme",
    sourceId: "industry-trends",
    researchType: "Industry trend",
    headline: "“AI teammate” is replacing “AI assistant” in your category's language",
    summary:
      "The phrasing flipped over about six weeks — in competitor copy, analyst notes and job titles. The words carry a different promise.",
    synthesis: [
      "Six weeks ago the category said assistant. It now says teammate, and the change is broad enough to show up in three independent places: competitor landing pages, two analyst notes, and the titles of roles being hired to own these features.",
      "The substitution is not cosmetic. An assistant waits to be asked and is judged on responses; a teammate is given work and judged on outcomes. The second promise is much harder to keep, and most of the copy using it hasn't earned it.",
      "That mismatch is the angle. Adopting the word without the outcome is the category's current position; being specific about which work is actually handed over, and what still isn't, is available and defensible.",
    ],
    posts: [
      {
        id: "rfp5-p1",
        network: "linkedin",
        publishedOn: "Jul 21, 2026",
        author: { name: "Field Notes Weekly", handle: "Field Notes Weekly", initials: "FN", accent: "soft-blue" },
        text: "Counted it: 14 of 20 vendor pages in this category said “assistant” in May. Nine say “teammate” now. Same feature set.",
        likes: 2360,
        comments: 198,
        reposts: 470,
      },
      {
        ...
        author: { name: "Marcus Oyelaran", handle: "@marcus_ops", initials: "MO", accent: "orange" },
        text: "If it's a teammate, tell me what it owns and who it reports to. If it's an assistant, stop charging like it's headcount.",
        likes: 3480,
        comments: 402,
        reposts: 690,
      },
    ...
    ideaSeeds: [
      {
        title: "What it owns, and what it doesn't",
        body: "List the work actually handed over and the work still yours. The honest boundary is the differentiator.",
        kind: "insight",
        rationale: "Nine vendors adopted the word this quarter; specificity is what none of them offered with it.",
        channels: ["linkedin"],
      },
      {
        title: "Assistant, teammate, headcount",
        body: "Use Marcus's framing to make a pricing argument: what a teammate would have to own to be priced like one.",
        kind: "hook",
        rationale: "The sharpest post in the set, and it lands on an objection your buyers already voice.",
        channels: ["linkedin", "x"],
      },
    ],
    scannedAt: "just now",
  ...
  {
    id: "rf-september-early",
    dedupeKey: "back-to-work-september-lands-early",
    contextId: "ctx-acme",
    sourceId: "global-trends",
    researchType: "Cultural moment",
    headline: "The back-to-work reset is landing three weeks early this year",
    summary:
      "Planning-and-reset conversation that usually peaks in early September is already climbing. The window for a planning piece is now, not late August.",
    synthesis: [
      "Search interest and posting volume around team planning, process resets and Q4 goals normally ramp in the last week of August and peak in the first week of September. Both curves started climbing in mid-July this year and are already near where they usually sit at the end of August.",
      "The likely cause is scheduling rather than mood — a compressed European holiday pattern and several category conferences moved earlier — but the effect on timing is the same regardless of cause.",
      "For a brand whose audience plans in that window, publishing to the old calendar means arriving after the decisions are made. A planning piece has about three more weeks of usefulness than usual, and then noticeably less.",
    ],
    posts: [
      {
        id: "rfp6-p1",
        ...
        publishedOn: "Jul 22, 2026",
        author: { name: "Priya Raman", handle: "Priya Raman", initials: "PR", accent: "green" },
        text: "Doing Q4 planning in July feels wrong and I'm doing it anyway — everyone I need in the room is away in August.",
        likes: 3120,
        comments: 288,
        reposts: 410,
      },
      {
        ...
        publishedOn: "Jul 19, 2026",
        author: { name: "Ops Weekly", handle: "Ops Weekly", initials: "OW", accent: "soft-blue" },
        text: "Reset season started early. Three planning threads on my timeline this week and it's the third week of July.",
        likes: 1640,
        comments: 122,
        reposts: 248,
      },
    ...
    ideaSeeds: [
      {
        title: "Plan Q4 before August takes the room",
        body: "The timing argument first — why the planning window moved — then the three decisions worth making before people leave.",
        kind: "insight",
        rationale: "The moment is already climbing; publishing to the old calendar arrives after the decisions.",
        channels: ["linkedin"],
      },
    ],
    scannedAt: "just now",
  ...
  {
    id: "rf-time-to-first-value",
    dedupeKey: "team-notes-point-at-time-to-first-value",
    contextId: "ctx-acme",
    sourceId: "team-ideas",
    researchType: "Internal priority",
    headline: "Your team's Q3 notes point at one unclaimed message: time to first value",
    summary: "Three planning docs and a launch brief name the same metric. It has never appeared in a post.",
    synthesis: [
      "Across the Q3 planning notes, one number recurs without being assigned to anyone: how long a new team takes to get something useful out of the product. It's in the launch brief as a goal, in the CS notes as a diagnosis, and in the roadmap as a justification.",
      "It has never been said publicly. Every external message this quarter has been about breadth — what the product covers — while the internal conversation is entirely about speed to the first useful output.",
      "Aligning the two is cheap: the argument is already written, agreed internally, and supported by numbers the team already trusts.",
    ],
    posts: [],
    ideaSeeds: [
      {
        title: "The number we plan around and never mention",
        body: "Publish the time-to-first-value target, why it's the metric, and what you changed to move it.",
        kind: "stat",
        rationale:
          "Three internal docs agree on this and no external post has used it — the cheapest alignment available.",
        channels: ["linkedin"],
      },
      {
        title: "Breadth is not the promise",
        body: "Argue against your own external message: coverage matters less than how fast the first useful thing happens.",
        kind: "insight",
        rationale: "Turns an internal consensus into a position, which is the shape this category rewards right now.",
        channels: ["linkedin", "x"],
      },
    ],
    scannedAt: "just now",
  ...
  {
    id: "rf-founder-numbers",
    dedupeKey: "founders-posting-one-real-number",
    contextId: "ctx-founder-voice",
    sourceId: "competitor-posts",
    researchType: "Creator signals",
    headline: "Founders posting one real number are outperforming founders posting frameworks",
    summary:
      "The pattern holds across the set: a single disclosed number beats a well-built framework by a wide margin, even when the number is unflattering.",
    synthesis: [
      "Comparing framework posts against disclosure posts among the founders your audience follows, the disclosures win consistently — one real number, stated without a chart, tends to reach several times further than a structured lesson.",
      "Unflattering numbers do best of all. Churn nobody asked about, a runway figure, a hiring mistake with a cost attached. The reach appears to come from the fact that the number could not have been invented.",
      "The Playbook already rules out promotional content and asks for lived experience. A disclosed number is the shortest possible form of both.",
    ],
    posts: [
      {
        id: "rfp8-p1",
        ...
        publishedOn: "Jul 23, 2026",
        author: { name: "Elena Fisk", handle: "Elena Fisk", initials: "EF", accent: "purple" },
        text: "We lost 14% of accounts in the quarter we raised prices. Kept the pricing. Here's what the 14% had in common, which we should have known.",
        likes: 9200,
        comments: 802,
        reposts: 1610,
      },
      {
        id: "rfp8-p2",
        network: "x",
        publishedOn: "Jul 10, 2026",
        author: { name: "Ravi Chaudhary", handle: "@ravibuilds", initials: "RC", accent: "green" },
        text: "Eleven months of runway and a hire I'd already promised. I took the hire. Still not sure it was right.",
        likes: 4700,
        comments: 512,
        reposts: 640,
      },
    ...
    ideaSeeds: [
      {
        title: "One number, no chart",
        body: "Pick the number you'd rather not publish, state it plainly, and say what it had in common.",
        kind: "stat",
        rationale: "Disclosures outperform frameworks across the whole set, and unflattering ones outperform the rest.",
        channels: ["linkedin"],
      },
      {
        title: "The decision I'm still unsure about",
        body: "A call you made without resolution. No lesson at the end.",
        kind: "story",
        rationale: "Reversals and open questions are the two shapes that can't be written by someone who wasn't there.",
        channels: ["linkedin", "x"],
      },
    ],
    scannedAt: "just now",
