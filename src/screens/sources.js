...
import { postDraftResult } from "../assistant.js?v=25";
  ...
  root.addEventListener(
    ...
    (event) => {
      ...
      if (clipsBtn) {
        ...
        if (!src) return;
        // startClipExtraction kicks off a non-blocking 30s extraction if the
        // source has no clips yet (toast on start, toast with "Open clips"
        // action when ready). If clips already exist, onReady is called
        // synchronously and the modal opens immediately.
        startClipExtraction(src, {
          onReady: (ready) =>
              ...
              onUseClips: (selectedClips, source) => {
                openChatPickerModal({
                  onPick: (pick) => {
                    ...
                    showToast(
                      `Drafted ${drafts.length} post${drafts.length === 1 ? "" : "s"} from ${source.filename}`,
                      {
                        duration: 3200,
                      },
                    );
