...
> **Vocabulary — UI label vs code name.** A saved AI context is a **Playbook** in the UI but a **Context** in code (`contexts-store`, `contextId`). Content Research is now **Content Ideas** in the UI: a lane is an **Idea stream** and each card in it is an **Idea**. Everything in code still says research / brief / lane / topic (`briefs-store`, `brief-card`, `data-brief-*`, `/content-ideas` routes, `.topics-*` classes) — same split as Playbook/Context, and `briefs-store` could not be renamed regardless because `topics-store` is a different feature. Source → Idea → Draft (post) → Schedule is the content pipeline.
>
> ⚠️ **"Idea" now names two objects; "Topic" is back to naming one.** The Topic→Idea rename cleared the old clash — `topics-store` + `/topics` (flag `topics`) own a **listening dossier**, still a **Topic**, and no Content Ideas surface says that word any more. The word carrying two meanings is now **Idea**: the pipeline's extracted Idea (right panel, `library.js`, `idea-card`) versus an Idea in a stream (`briefs-store`, `brief-card`). Both are user-visible, so name the surface when it could be either — "an Idea in this stream", not a bare "Idea". In code the two never touch: different stores, different card modules, different routes.
