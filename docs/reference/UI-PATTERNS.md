...
| `.ap-infobox.main`                                      | Variante orange. Le DS n'a que info/warning/error/success — tous sous-évaluent une notice qui n'est ni une mise en garde ni un aparté neutre. `warning` refusé : bon ton, mauvaise raison. Filet + icône = `--sys-color-main-light-text` ; **fond = `--app-peach`**, la même surface que `.brief-card__whynow` au-dessus duquel la notice se pose (même précédent que `.ap-status.mermaid` avec `--app-butter`). |
