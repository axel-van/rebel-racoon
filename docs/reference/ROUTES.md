...
- `/topics?view=sources&pb=ctx-…` — l'onglet config **scopé** au même Playbook. `?pb=` est une seule idée partagée par les deux onglets ; seule la valeur absente diffère (feed → tous, config → le défaut ★). Obligatoire sur la config : sans lui, configurer B puis Retour montrerait A.
