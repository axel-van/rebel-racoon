...
| **Share Playbook** ([`share-playbook-modal.js`](../../src/components/share-playbook-modal.js))       | Flag `playbookSharing` : la forme Notion / Slite — un champ « Add a teammate », la liste **People with access** (propriétaire en première ligne), la switch-row **General access**, l'avertissement de conséquence en sortie de partage, puis derrière un filet le transfert de propriété et le journal des modifications (disclosures). Les trois portées du store se **dérivent** de ces deux contrôles, elles ne se choisissent plus. Voir §9bis. |
