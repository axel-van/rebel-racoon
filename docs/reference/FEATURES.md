...
- **Le modal** ([`share-playbook-modal.js`](../../src/components/share-playbook-modal.js), 560px) — **la forme de Notion / Slite / Drive**, et pour la même raison : la question avec laquelle on ouvre ce dialogue est « qui a ça ? », pas « dans lequel de trois modes est-ce ? ». Donc la réponse est ce qu'on voit en premier. Dans l'ordre : un **champ pour ajouter un collègue**, la liste **People with access**, le select **General access**, l'`.ap-infobox.warning` de conséquence, puis derrière un filet le **transfert**, replié. Ouvert depuis la carte et depuis la fiche. CTA **bleu** : partager est une action de liste routinière, pas une action IA.
