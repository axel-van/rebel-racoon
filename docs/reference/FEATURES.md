...
- **Le modal** ([`share-playbook-modal.js`](../../src/components/share-playbook-modal.js), 560px) — trois `.ap-radio-card`, l'`.ap-infobox.warning` de conséquence, puis, **derrière un filet**, ce qui parle de l'objet et non de la décision : la ligne **Owner**, le **transfert** et **Recent changes**, chacun replié dans une disclosure (le modal porte une décision ; la gouvernance est à un clic). Ouvert depuis la carte et depuis la fiche. CTA **bleu** : partager est une action de liste routinière, pas une action IA.
  - **Le contenu SCROLLE** (`overflow-y: auto` + `min-height: 0` sur `.ap-dialog-content`) et les deux `.ap-select` s'ouvrent **dans le flux**. ⚠️ C'était l'inverse — `overflow: visible` sur le contenu pour laisser les popovers s'échapper de la modale — et c'était mesurable : les deux disclosures ouvertes sur un viewport de 820px, le footer Cancel / Save tombait **119px sous l'écran**, injoignable, pendant que le panneau du people-picker flottait hors de la silhouette du dialogue par-dessus lui. Un panneau en flux POUSSE au lieu de cacher, et un dialogue qui scrolle garde toujours son footer. Ne pas re-repositionner ces deux dropdowns en `absolute`.
  - **Le rang, pas l'empilement.** Le titre d'une carte sort en grey-100 et son hint en grey-80 (le DS les rendait tous les deux en grey-80 — voir le patch `.ap-radio-card-title` dans [`UI-PATTERNS.md`](UI-PATTERNS.md) §1), la sélection est doublée par un anneau `inset` qui ne déplace rien, et le bas de la modale — trois lignes de gras identique auparavant — devient un label `Owner` en 12px grey-60 au-dessus du nom, puis deux disclosures pleine largeur (glyphe, libellé, chevron à l'extrémité) qui ressemblent aux contrôles qu'elles sont.
  - **La phrase commune est dite UNE fois**, sous le groupe (`reachNote`) : « Whoever it reaches can read it and write with it. Only {you|Owner} can change what it says. » ⚠️ Elle était répétée **mot pour mot dans les deux cartes de partage** — un tiers des mots du dialogue en doublon, qui enterrait la distinction fixe ⇄ dynamique que ces cartes existent pour tracer — et elle était **fausse** sur l'écran d'un manager, qui gouverne sans jamais pouvoir éditer. D'où le nom du propriétaire plutôt que « you ».
  - **Le picker est rendu DANS le groupe de radios, juste sous la carte « Specific people »** — il s'affichait après la troisième carte, donc le seul contrôle du dialogue pendait sous la mauvaise option et un `margin-left` tentait de suggérer ce que la proximité aurait dû dire. 4px de la carte à laquelle il appartient contre 8px entre options : c'est toute la grammaire du bloc.
  - **Le people picker est le Selection Dropdown de l'ADS** derrière un trigger `.ap-select` — le composant que le DS livre pour ça (recherche + items cochables + état vide), et **la même composition que le picker de dossiers** de la modale save-drafts, dont il partage les règles de positionnement du popover. Un seul patron pour « choisir dans une liste » en modale, pas deux. Le trigger dit qui est dedans en une ligne : `.ap-avatar-group` (3 visages max + la bulle `+N` du DS) puis les noms — deux tiennent, au-delà un compte se lit mieux qu'une liste tronquée. Le panneau ne s'ouvre **d'emblée que** quand personne n'est encore choisi : c'est l'état où il faut l'ouvrir de toute façon ; avec une liste déjà dans le trigger il arrive replié. Il s'ouvre **en flux**, prolongeant le trigger (bord partagé, pas de radius haut, pas d'ombre — une élévation dans une surface déjà élevée lirait deux plans là où il n'y en a qu'un) et scrolle en interne à 220px. La recherche **plie les accents** des deux côtés, sinon « lea » manque Léa Mercier et « ines » manque Inès Ferrand.
  - ⚠️ **Ça a été une liste plate de onze cartes cochables**, empilées dans le corps de la modale, pendant un commit : ça rendait le contenu ~250px plus haut qu'un viewport de 900px ne peut tenir (Save passait sous le pli) et ça demandait de scanner un mur de lignes pour répondre « qui l'a ? », ce que le trigger dit maintenant en une ligne. Ne pas y revenir.
  - **Cocher quelqu'un ne re-render pas** : `syncCommit()` + `refreshConsequence()` retouchent le bouton et l'encart, sinon le curseur et la requête de recherche partiraient à chaque clic. Changer de carte, si : c'est structurel (le picker apparaît ou s'en va), et le focus est remis sur la carte cochée.
  - **Save ne dit qu'une chose à la fois** — « Share with the org » / « Share with 3 people » / « Make it private », et **« Pick who gets it »** grisé quand la liste est vide : décocher tout le monde n'est pas une façon de repasser en privé.
  - **La conséquence ne sort qu'en RÉTRÉCISSEMENT** de portée, et **nomme** qui perd l'accès (au-delà de deux : « X, Y et 2 others » — un avertissement qui liste onze noms n'est plus lu). Partager plus large ne coûte rien à personne, donc rien ne s'affiche.
...
| **Share Playbook** ([`share-playbook-modal.js`](../../src/components/share-playbook-modal.js))       | Flag `playbookSharing` : trois radio-cards (moi seul / des gens nommés / toute l'org) + le people picker sous la carte du milieu, l'avertissement de conséquence en sortie de partage, puis derrière un filet la ligne Owner, le transfert de propriété et le journal des modifications (disclosures). Voir §9bis. |
