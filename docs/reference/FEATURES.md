...
- **Le modal** ([`share-playbook-modal.js`](../../src/components/share-playbook-modal.js), 560px) — **la forme de Notion / Slite / Drive**, et pour la même raison : la question avec laquelle on ouvre ce dialogue est « qui a ça ? », pas « dans lequel de trois modes est-ce ? ». Donc la réponse est ce qu'on voit en premier. Dans l'ordre : un **champ pour ajouter un collègue**, la liste **People with access**, la switch-row **General access**, l'`.ap-infobox.warning` de conséquence, puis derrière un filet le **transfert**, replié. Ouvert depuis la carte et depuis la fiche. CTA **bleu** : partager est une action de liste routinière, pas une action IA.
  ...
  - **`General access` est le seul endroit où la couture FIXE ⇄ DYNAMIQUE se trace** (doc §5.1) : la liste au-dessus est un ensemble de visages nommés qu'on peut compter, ceci est une règle qui **suit** l'org. C'est **une switch-row** — glyphe, libellé, `.ap-toggle-container`, et sous elle la ligne qui dit la portée réelle (le compte, ou le compte social quand la fiche est liée à un profil) — la même forme que `/topics/settings` donne à chaque source d'écoute. ⚠️ **C'était un `.ap-select` de DEUX options, et le cliquer poussait la modale** : un dropdown dans un conteneur qui scrolle ne peut pas flotter (le scroller le découpe), donc il devait s'ouvrir dans le flux — ce qu'un select ne fait pas. Un choix binaire n'a pas besoin d'un menu.
