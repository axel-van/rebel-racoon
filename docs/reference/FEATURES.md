...
| `topics`                 | Topics (listening dossiers)     | **OFF** | Toute la feature **Topics** (§17) : la route `/topics` + son entrée de nav et son compteur d'unseen, la dialog du dossier, et l'onglet **What I watch**. La donnée (dossiers seedés + `ctx.topics`) reste présente quand OFF, comme `playbookCompetitors`. |
...
DS `.ap-tabs` : **Feed** (avec son compteur) | **What I watch** (sans compteur — un nombre y serait ambigu : des sources ? des Playbooks ?). L'onglet actif vit dans **`?view=sources`**. **`?pb=` est UNE seule idée partagée par les deux onglets** — « scopé à ce Playbook » : filtrer le feed sur une marque puis passer à What I watch atterrit sur la config de cette marque. Les onglets ne diffèrent que sur ce qu'une valeur **absente** veut dire, et ça découle de leur rôle : feed → tous les Playbooks ; config → le Playbook par défaut (★), puisqu'on édite toujours quelque chose. Un seul `scopedQuery()` construit le hash pour les bascules d'onglet, les choix de Playbook et le Clear, pas en state module : `renderTopics` réinitialise `view` à chaque render et le router re-run le handler sur un changement de query seul, donc un onglet en state module reviendrait aussitôt au feed. Dans l'URL il est en plus deep-linkable et le retour navigateur fait ce qu'on attend. **Refresh now** n'existe que sur le feed — rien à rafraîchir sur l'onglet config, et ça scannerait des sources en cours d'édition.
...
- **Filtres — deux facettes derrière un seul déclencheur** (`Playbook` · `Source`), sur le panneau DS `.ap-selection-dropdown` (search · groups · items · footer). C'est **la règle de filtrage du DS** : bascules toujours visibles → _filter chips list_ ; **options groupées derrière un déclencheur → _filter dropdown_**. Deux facettes dont une aussi longue que le compte a de Playbooks, c'est clairement la seconde. `<ap-filter-dropdown>` est le composant Angular correspondant mais **ne ship aucune couche CSS-UI** — ce panneau est son équivalent CSS-UI, et c'est **sa première utilisation dans l'app**. Une rangée de puces Playbook était l'alternative : c'est le piège dont l'onglet de config vient de sortir, elle ne survit pas à vingt Playbooks.
  - **Les compteurs de chaque facette sont calculés contre la sélection de l'AUTRE**, donc un nombre ne promet jamais des lignes que les filtres en cours excluraient (Pawtrack + toutes sources = 7, et ses six compteurs par source font exactement 7).
  - **Un compteur à zéro _désactive_ la ligne au lieu de la masquer** — la liste ne se réorganise pas sous le curseur. Effet de bord voulu : une combinaison morte est **inatteignable** par l'UI.
  - **Seuls les Playbooks présents dans le feed ont une ligne** — un filtre qui ne peut que ne rien renvoyer n'est pas un filtre. C'est ce qui règle vraiment l'échelle : à 14 Playbooks le panneau en listait 3. La dalle de recherche du DS apparaît **au-delà de 8 lignes** et filtre les deux groupes à la fois, en-têtes de groupe maintenus en place.
