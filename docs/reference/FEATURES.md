...
Bloc propre **au-dessus** de la grille de workflows, sous son propre `empty-chat__starter-label` _« Topics waiting for you »_, à la **largeur des trois starter cards** (1040px, 651px sous 900px — la même que la grille, donc un seul alignement).

- La file vient de `briefs-store.getStarterTopics()` : **4 Topics** max, ordonnées trending → updated → les plus récentes non triées. Statuts Saved / Used / Ignored exclus (déjà répondus).
- **Slides = file + 1** : la dernière est la carte de clôture _« Get the full details into your Topic feeds »_ / _« Browse all your topic feeds in one place, deep dive into ideas. »_ + « Explore more ideas ». Elle a **son propre point**, donc chaque point correspond à une slide.
...
- **Anneau dans les deux sens** : aucun chevron n'est jamais désactivé. Suivant depuis la dernière slide → la première Topic ; Précédent depuis la première → la carte de clôture. Les deux gardent leur chevron sur toutes les slides.
...
- Avant la 1ʳᵉ Topic : carte d'attente (3 s, spinner DS). Aucune Topic du tout (`new-alt`) : **pas de bloc**, ni libellé ni carte de clôture.
...
| 7   | 5 points pour 4 Topics + la carte de clôture ; un point = une slide                                                                                      | ✅                                                                                                          |
...
| 10  | La carte dit **« Read the full idea »** et l'ouvre en dialog : même corps et mêmes trois verbes que le panneau du feed                                   | ✅ Full Topic · Trend levels · Topic history · Sources ; pied `topics-use-flat`                             |
| 11  | Dans le dialog : Save bascule le libellé sans fermer, Ignore ouvre le dialog de raison, Use in chat ferme, marque `used` et ouvre un chat avec la Source | ✅ toast « Saved for later » ; « Why did this Topic miss the mark? » ; l'Topic utilisée quitte le carrousel |
| 12  | Flag OFF : aucune surface Topic feeds, deep-link périmé → `/`                                                                                            | ✅                                                                                                          |
