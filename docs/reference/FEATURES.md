...
- **Le modal** ([`share-playbook-modal.js`](../../src/components/share-playbook-modal.js), 560px) — **la forme de Notion / Slite / Drive**, et pour la même raison : la question avec laquelle on ouvre ce dialogue est « qui a ça ? », pas « dans lequel de trois modes est-ce ? ». Donc la réponse est ce qu'on voit en premier. Dans l'ordre : un **champ pour ajouter un collègue**, la liste **People with access**, la switch-row **General access**, l'`.ap-infobox.warning` de conséquence, puis derrière un filet le **transfert**, replié. Ouvert depuis la carte et depuis la fiche. CTA **bleu** : partager est une action de liste routinière, pas une action IA.
  ...
  - **DEUX positions en x dans tout le dialogue, et seulement deux.** Les intitulés de section pendent à 0, tout ce qu'une ligne dit commence à `--share-indent` (32px = la gouttière de 24 + 8). ⚠️ Avant ça l'avatar faisait 24px, le glyphe d'accès général 16 et celui de la disclosure 16 : trois familles de lignes démarraient leur texte à trois x différents, ce qui est ce qui faisait lire un dialogue de cinq lignes courtes comme des débris flottants. Chaque graphique de tête occupe désormais la MÊME fente, quelle que soit sa taille propre — avatar, `__orb`, `__fold-glyph`. Et **une seule hauteur de ligne, 40px**, pour les personnes, l'accès général et la disclosure : répéter la même bande est tout le rythme vertical, les filets entre personnes ne font que le rendre visible. Les deux variables vivent sur `.share-playbook-modal`, pas en dur.
