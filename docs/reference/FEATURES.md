...
- **Deux cartes** — **Refresh** (une ligne label + `.ap-select`) et **Sources** (les six en **lignes** dans une seule carte, séparées par des filets, avec le switch DS à droite). Des lignes et pas une grille : à 700px une grille donne deux colonnes serrées, et une liste avec l'interrupteur à droite est la forme canonique d'un réglage. Les OFF sont **atténuées**, pas creusées — il n'y a plus de gap de grille pour porter un fond.
