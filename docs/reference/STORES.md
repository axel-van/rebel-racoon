...
| [`library.js`](../../src/library.js)                     | Ideas per-session **+ le pool global** (`mocks.ideas`) que lisent le panneau de droite, `draft-flow` et `/ideas`. `poolAdd`/`poolRemove` tiennent les deux d'accord : avant, seul `injectIdeasForSource` y écrivait et aucune suppression n'y purgeait, donc une surface globale montrait des fantômes. Une idée porte `origin` (`"session"` \| `"research"`) et son `sessionId` (`null` si elle ne vient d'aucune conversation). | `getSources(sid)`, `getIdeas(sid)`, `getAllIdeas({origin})`, `getIdeaById`, `countIdeasByOrigin`, `addGlobalIdeas`, `adoptIdea`, `removeIdeasGlobally`, `appendExtractedIdeas`, `injectIdeasForSource`, `extractVideoIdeas`, `removeIdeas`, `subscribe(sid, fn)`, `subscribeGlobal(fn)` |
