...
| [`research-store.js`](../../src/research-store.js)       | **Global** : les **Topic feeds** (lanes) — un Playbook × un jeu de sources × une cadence (flag `contentResearch`).                                                                                                                                                                            | `getLanes`, `getLaneById`, `addLane`, `updateLane`, `duplicateLane`, `deleteLane`, `subscribe`                                                                                                                                                                 |
| [`briefs-store.js`](../../src/briefs-store.js)           | **Global** : les **Topics** d'un feed. Le triage (`new` / `saved` / `used` / `ignored`), les deux signaux (trending, updated) et les versions d'article vivent ici ; `getStarterTopics()` sert les 4 Topics du carrousel de la page nouvelle session.                                         | `getBriefById`, `getBriefsForLane`, `getStarterTopics`, `getBriefVersions`, `groupBriefsByAge`, `attentionCountsForLane`, `defaultFilters`, `narrowedGroupCount`, `setStatus`, `toggleSaved`, `subscribe`                                                      |
...
`research-store` + `briefs-store` sont globaux pour exactement la raison de `topics-store` : un Topic feed tourne sur une cadence et ses Topics arrivent avant qu'un chat existe pour les accueillir. C'est ce qui permet au carrousel de la page nouvelle session de servir des Topics de **n'importe quel** feed, et au compteur de la nav de sommer tout le compte.
