...
| [`research-store.js`](../../src/research-store.js)       | **Global** : les **Idea streams** (lanes) — un Playbook × un jeu de sources × une cadence (flag `contentResearch`).                                                                                                                                                                           | `getLanes`, `getLaneById`, `addLane`, `updateLane`, `duplicateLane`, `deleteLane`, `subscribe`                                                                                                                                                                 |
| [`briefs-store.js`](../../src/briefs-store.js)           | **Global** : les **Ideas** d'un stream. Le triage (`new` / `saved` / `used` / `ignored`), les deux signaux (trending, updated) et les versions d'article vivent ici ; `getStarterTopics()` sert les 4 Ideas du carrousel de la page nouvelle session.                                         | `getBriefById`, `getBriefsForLane`, `getStarterTopics`, `getBriefVersions`, `groupBriefsByAge`, `attentionCountsForLane`, `defaultFilters`, `narrowedGroupCount`, `setStatus`, `toggleSaved`, `subscribe`                                                      |
...
`research-store` + `briefs-store` sont globaux pour exactement la raison de `topics-store` : un Idea stream tourne sur une cadence et ses Ideas arrivent avant qu'un chat existe pour les accueillir. C'est ce qui permet au carrousel de la page nouvelle session de servir des Ideas de **n'importe quel** stream, et au compteur de la nav de sommer tout le compte.
