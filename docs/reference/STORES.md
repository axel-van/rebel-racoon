...
| [`topics-store.js`](../../src/topics-store.js)           | **Global** : les dossiers du listening, tout le compte (flag `topics`). Masque au dismiss au lieu de supprimer, pour que le toast puisse vraiment offrir Undo. `refreshTopics()` drain un pool seedé et vieillit tout le reste d'un jour.                                                     | `getTopics`, `getTopicById`, `getUnseenCount`, `countBySource`, `markSeen`, `dismissTopic`, `restoreTopic`, `refreshTopics`, `hasMoreToScan`, `topicWhen`, `subscribe`                                                                                         |
| [`research-store.js`](../../src/research-store.js)       | **Global** : les **Inspiration feeds** (lanes) — un Playbook × un jeu de sources × une cadence (flag `contentResearch`).                                                                                                                                                                      | `getLanes`, `getLaneById`, `addLane`, `updateLane`, `duplicateLane`, `deleteLane`, `subscribe`                                                                                                                                                                 |
| [`briefs-store.js`](../../src/briefs-store.js)           | **Global** : les **Inspirations** d'un feed. Le triage (`new` / `saved` / `used` / `ignored`), les deux signaux (trending, updated) et les versions d'article vivent ici ; `getStarterTopics()` sert les 4 Inspirations du carrousel de la page nouvelle session.                             | `getBriefById`, `getBriefsForLane`, `getStarterTopics`, `getBriefVersions`, `groupBriefsByAge`, `attentionCountsForLane`, `defaultFilters`, `narrowedGroupCount`, `setStatus`, `toggleSaved`, `subscribe`                                                      |

## Les globaux : `sources-stream`, `topics-store`, `research-store`, `briefs-store`
...
`topics-store` est global pour une autre raison : un topic appartient à un **Playbook** et arrive sur une cadence, bien avant qu'un chat existe pour l'accueillir. Le feed `/topics` traverse tous les Playbooks, et le compteur de la sidebar somme délibérément tout le compte — l'arrivée est un évènement account-level même si la config qui l'a produite (`ctx.topics`) est par Playbook. Les catalogues (`connectors-store`, `topics-catalog`, `research-catalog`) sont eux aussi non-per-session, mais ce sont des listes de config, pas du contenu.

`research-store` + `briefs-store` sont globaux pour exactement la raison de `topics-store` : un Inspiration feed tourne sur une cadence et ses Inspirations arrivent avant qu'un chat existe pour les accueillir. C'est ce qui permet au carrousel de la page nouvelle session de servir des Inspirations de **n'importe quel** feed, et au compteur de la nav de sommer tout le compte.
...
```js
import { getUserMode, isNewUser, isNewUserAlt } from "./user-mode.js";
...
Switch UI : **popover ⚙️ Admin de la sidebar** (`admin-menu.js`) — il n'y a plus de route `/settings`. Un reload est forcé pour que les stores re-seedent.
