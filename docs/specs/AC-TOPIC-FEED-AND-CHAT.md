...
| `AC-ACT-2` | Ignore asks for a reason and is reversible   | **Ignore** asks for a reason. On submit, the Topic is Ignored and the reason is kept, and the Topic **leaves the feed** — Ignored is the one state unticked by default. Ignoring is reversible; it isn't deletion.                                                                                     |
...
| #   | Question                                                                                                                                                                                                                                  | Blocks                               |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| 1   | ~~Should the picker resurface ignored-but-trending Topics?~~ **Answered: no** — see `AC-PICK-2b`.                                                                                                                                         | `AC-PICK-2b`                         |
...
| 10  | ~~Nothing surfaces a spike on a Topic the reader has already triaged.~~ **Answered: that is the intent.** Ignore means ignore; re-ticking Ignored in the filter is the way to see one, and it keeps its Trending mark when it comes back. | `AC-SIG-1`, `AC-SIG-2`, `AC-PICK-2b` |
